<?php
 if(client_get_options('home-box-7')){
    $taxonomy_name = 'category';
    $home_category_7 = client_get_options('home-box-7');
    $home_category_7_list_post_new =  get_posts(array(
        'numberposts'   => 2, // get all posts.
        'tax_query'     => array(
            array(
                'taxonomy'  => 'category',
                'field'     => 'id',
                'terms'     => $home_category_7,
            ),
        ),
    ));
?>
<section class="home-box-4">
    <div class="panel panel-default category event">
        <div class="panel-heading">
            <a href="<?php echo get_category_link($home_category_7); ?>" title="<?php echo category_get_name_by_ID($home_category_7); ?>">
                <h2><?php echo category_get_name_by_ID($home_category_7); ?></h2>
             </a>
            <?php
            wp_nav_menu(array("menu" => client_get_options('home-box-7-sub-category-1'), 'container' => '', 'container_class' => '', 'menu_class' => 'nav nav-pills pull-right nomal'));
            ?>
        </div>
        <div class="panel-body">
            <?php foreach ($home_category_7_list_post_new as $key => $value) { ?>
